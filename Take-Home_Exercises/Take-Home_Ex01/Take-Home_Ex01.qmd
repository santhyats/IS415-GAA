...
battles_sf<-read_rds("data/rds/Battles/battles_sf.rds")
quart_geo_bat<- read_rds("data/rds/Battles/quart_geo_bat.rds")
...
exp_sf<-read_rds("data/rds/Explosions/exp_sf.rds")
quart_geo_exp<- read_rds("data/rds/Explosions/quart_geo_exp.rds")
...
strat_dev_sf<- read_rds("data/rds/Strat_Dev/strat_dev.rds")
quart_geo_strat<-read_rds("data/rds/Strat_Dev/quart_geo_strat.rds")
...
civViolence_sf<- read_rds("data/rds/Violence/civViolence_sf.rds")
quart_geo_civ<- read_rds("data/rds/Violence/quart_geo_civ.rds")
```
...
Violence Against the civilians can be observed in the central and southern regions of Myanmar throughout all the quarters, with the concentration once again, being at the central region. There appears to be a brief reduction in the intensity of the attacks in the 3rd quarter of 2022.
:::::::
...
battles_g = Gest(battles_ppp_com) 
battles_g.csr <- envelope(battles_ppp_com, Gest, nsim = 99)
plot(battles_g.csr)
```
...
1.  Maizland, L. (2022, January 31). Myanmar’s troubled history: coups, military rule, and ethnic conflict. *Council on Foreign Relations*. https://www.cfr.org/backgrounder/myanmar-history-coup-military-rule-ethnic-conflict-rohingya
